...
genPortfolio.bond <- function(n,mkt,dur,dur.sd,max.mat=30,type="random",f=.5,name=as.character(runif(1)),...) {
	...
	# Calculate constants
  mr <- market.rate(mkt,mat=max.mat)
	max.dur <- findMaxDur(i=mr,market.rate=mr)
	# Calculate portfolio
	...
	for(num in seq(n)) {
		...
		while(mat.Q>max.mat) {
			...
			requested.dur <- -99
			while(requested.dur<0.5 | requested.dur > max.dur*.85) { # The 90% in here is a kludgy fix to avoid getting near the maximum duration where things get unstable
				if(type=="random") {
